...
def test_load_plugins_from_config(cli_runner):
    _write_config(
        {'plugins': ['tests.cli_tests.test_cli_plugins:DummyTestPlugin']}
    )
...
def test_disable_non_default_plugins(cli_runner):
    _write_config(
        {
            'plugins': [
                'tests.cli_tests.test_cli_plugins:DummyTestPlugin',
            ]
        }
    )
    result = cli_runner.invoke(
...
def test_show_unavailable_plugins(cli_runner):
    _write_config(
        {
            'plugins': [
                'tests.cli_tests.test_cli_plugins:UnavailablePlugin',
            ]
        }
    )
    result = cli_runner.invoke(
