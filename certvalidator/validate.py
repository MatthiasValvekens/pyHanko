...
def verify_ocsp_response(cert, path, validation_context, cert_description=None, end_entity_name_override=None):
    ...
    for ocsp_response in ocsp_responses:
        ...
        if moment < cert_response['this_update'].native - tolerance:
            failures.append((
...
def verify_crl(cert, path, validation_context, use_deltas=True, cert_description=None, end_entity_name_override=None):
    ...
    while len(crls_to_process) > 0:
        ...
        tolerance = validation_context.time_tolerance
        if moment < certificate_list['tbs_cert_list']['this_update'].native - tolerance:
            failures.append((
                'CRL is from after the validation time',
                certificate_list
            ))
            continue
        if moment > certificate_list['tbs_cert_list']['next_update'].native + tolerance:
            failures.append((
        ...
        if use_deltas and delta_certificate_list:
            ...
            if moment < delta_certificate_list['tbs_cert_list']['this_update'].native - tolerance:
                failures.append((
                ...
                continue
            if moment > delta_certificate_list['tbs_cert_list']['next_update'].native + tolerance:
                failures.append((
