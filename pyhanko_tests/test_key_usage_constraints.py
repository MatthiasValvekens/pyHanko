...
@pytest.mark.parametrize('cfg, cert_ku', [
    ...
    (
            KeyUsageConstraints(key_usage=set()),
            KeyUsage({'non_repudiation'})
    ),
    (
...
@pytest.mark.parametrize('cfg, cert_eku', [
    (
    ...
    (
        KeyUsageConstraints(extd_key_usage={'time_stamping'}),
        ExtKeyUsageSyntax(['time_stamping', 'microsoft_time_stamp_signing']),
    ),
    (
        KeyUsageConstraints(
    ...
    (
        ...
        KeyUsageConstraints(), ExtKeyUsageSyntax(['client_auth']),
    ),
    (
        KeyUsageConstraints(extd_key_usage={'microsoft_document_signing'},
                            explicit_extd_key_usage_required=False),
...
@pytest.mark.parametrize('cfg, cert_eku, err', [
    (
        KeyUsageConstraints(extd_key_usage={'time_stamping'}),
        None,
